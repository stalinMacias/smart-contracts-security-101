...
describe("Tx.origin", function () {
  ...
  beforeEach(async function () {
    ...
    // send 10000 weis to the SmallWallet contract
  ...
  describe("SmallWallet", function () {
    it("Should accept deposits", async function () {
      // Get the SmallWallet's contract ETH balance
      expect(await ethers.provider.getBalance(this.smallWallet.address)).to.eq(10000);
    });
    
    it("Should allow the owner to execute withdrawAll and send the funds to the User", async function () {
  ...
  describe("Attack", function () {
    it("Should drain the victim out if smallWallet's owner sends ether", async function () {
      const initialAttackerAccountBalance = await ethers.provider.getBalance(attacker.address);
      const initialVictimBalance = await ethers.provider.getBalance(this.smallWallet.address);

      // SmallWallet's Owner is the deployer account -> Sending ETH as the deployer user to the Attacker's contract
      // deployer account send 1 wei to the Attacker contract
      await deployer.sendTransaction({ to: this.attackerContract.address, value: 1 });

      expect(await ethers.provider.getBalance(this.smallWallet.address)).to.eq(0);
      // The ETH withdrawn from the SmallWallet contract should be transfered to the Attacker's account
      expect(await ethers.provider.getBalance(attacker.address)).to.eq(initialAttackerAccountBalance.add(initialVictimBalance));

      // The 1 wei that was sent to the Attacker contract should be stuck there!
      expect(await ethers.provider.getBalance(this.attackerContract.address)).to.eq(1);
    });
  });
